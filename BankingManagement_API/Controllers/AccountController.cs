...
public class AccountController : ControllerBase
{
    ...
    [HttpPut("deposit/{rib}")]
    public ActionResult Deposit(string rib, TransactionRequest request)
    {
        if (rib != request.Rib)
        {
        ...
        Account accountToUpdate = _accountService.GetAccountByRib(rib);
        ...
        _accountService.UpdateAccount(accountToUpdate.RIB, request.Ammount, OperationType.debit);
        return NoContent();
    }
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [HttpPut("credit/{rib}")]
    public ActionResult Credit(string rib, TransactionRequest request)
    {
        if (rib != request.Rib)
        {
            return BadRequest("Ribs does not match!");
        ...
        Account accountToUpdateDto = _accountService.GetAccountByRib(rib);
        ...
        _accountService.UpdateAccount(accountToUpdateDto.RIB, request.Ammount, OperationType.credit);
        return NoContent();
    }
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [HttpGet("history/{rib}")]
    public ActionResult GetAccountHistory(string rib)
    {
        ...
        Account accountHistory = _accountService.GetAccountHistory(rib);
